...
def test_source_branch_if_MR_present(caplog):
    ...
    with patch("gitlab.Gitlab", MagicMock()) as mocked_gl:
        ...
        class MR:
            def __init__(self, description, source_branch, target_branch):
                self.title = ""
        ...
        mocked_gl().projects().get().mergerequests.list = MagicMock(
            return_value=[MR(description, source_branch, target_branch)]
        )
        assert (
            get_source_branch_if_MR_present(
                mocked_gl().projects().get(), component_url, target_branch
            )
            == source_branch
        )
    ...
    with patch(
        ...
        with patch(
            ...
            with patch(
                ...
                git.Repo.clone_from().git.push.assert_not_called()
    ...
    with patch(
        ...
        with patch(
            ...
            with patch("update_components.update_component", MagicMock()):
                ...
                gitlab.Gitlab().projects.get().mergerequests.create.assert_not_called()
                git.Repo.clone_from().git.push.assert_called_with(
    ...
    with patch(
        ...
        with patch(
            ...
            with patch("update_components.update_component", MagicMock()):
                ...
                git.Repo.clone_from().git.push.assert_called_once()
...
def test_update_and_create_mrs_mr_exists_and_no_new_changes(caplog):
    ...
    git.Repo = MagicMock()
    gitlab.Gitlab = MagicMock()
    git.Repo.clone_from().git.diff = MagicMock(return_value="")
