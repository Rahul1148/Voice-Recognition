...
def check_for_cmake_tpip_changes(file, file_url, change):
    """
    Checks for changes to FetchContent_Declare in a cmake file.
    There are several things that could impact TPIP:
    1. The addition or removal of a FetchContent_Declare
       function.
    2. The modification of a FetchContent_Declare function where
       the URL is changed.
    """

    fetch_changed_re = re.compile(r"[-\+] *FetchContent_Declare")
    ...
    for line in change["diff"].splitlines():
        ...
        # Check file changes and look for FetchContent_Declare
        if fetch_changed_re.match(line):
            # Fetch content has been removed
            comment += f"\n[{file}]({file_url})\n"
            comment += f"\n- {current_line_marker}\n"
            comment += "\n- FetchContent_Declare has been modified.\n"

        comment += check_line_for_url_match(line, file, file_url, current_line_marker)
...
def check_file_changes(change, path_base, server_url):
    ...
    file_url = urljoin(server_url, file_path)
    ...
    if ".gitmodules" in file:
        ...
        if ".cmake" in file:
            comment += check_for_cmake_tpip_changes(file, file_url, change)
...
def check_files_for_tpip(server_url, private_token, project_id, merge_req_id, verbose):
    ...
    # 1. Flag up addition/removal of FetchContent_Declare in .cmake files
    # 2. Flag up any changes to .gitmodules
    # 3. In other source files check if an 'http' address has been added
    #    or removed.
    #
    ...
    path_base = os.path.join(
        rel_repo_path, "-/blob", merge_req_changes["source_branch"]
    )
